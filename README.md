...
The investigation utilised the BOTSv3 dataset to simulate a realistic cloud centric security incident within Frothlys AWS environemnt, analysed using Splunk as a SIEM platform. Through structureed log analysis aligned to the cyber kill chain and SOC operational practices, this couresework demonstrated how cloud telemetry, endpoint monitoring and access logging can be leveraged to identify misconfigurations, insecure authentication practices and anomalous activity.
The analysis successfully identified IAM user activity within the AWS environment, highlighted the detection of AWS API activity occuring without MFA, and investigated a critical S3 bucket misconfiguration that resulted in public accessibility. By correlating CloudTrail events with S3 access logs, the investigation confirmed that a text file was uploaded while the bucket was publicly accessible, demonstrating the importance of defence in depth when securing cloud storage services. Endpoint telemetry futher revealed deviations in operating systems accross hosts, reinforcing the value of asset baseline monitoring within SOC environments.
From a SOC perspective, this exercise closely mirrors real world Tier 1 and Tier 2 analyst responsibilities. Initial detection relied on log triage and keyword based searching, escalation involved contextual enrichment and correlation across multiple data sources and response considerations focused on access control remediation, identity hardening and visibility improvements. The investigation emphasised the importance of MFA enforcement, continuous monitoring of cloud configuration changes and the integration of multiple log sources to reduce blind spots.
Several strategic imporvements emerge from this analysis. Proactive alerting on non MFA API calls, automated detection of public S3 ACL changes and tighter IAM privelege reviews would significantly reduce the attack surface. Additionally, standardising endpoint configurations and improving log coverage across cloud services would enhance detection fidelity and incident response efficiency.
Overall this coursework demonstrated the practical application of SOC methodologies, Splunk based threat investigation and cloud security analysis in a realistic enterprise scenario. The BOTSv3 dataset provided valuable insight into how misconfigurations and weak identity controls can be exploited, while reinforcing the critical role of SOC analysts in detection, analysing and responding to modern cloud based threats.
